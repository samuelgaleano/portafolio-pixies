  ...
  return (
    <footer className="mt-24 border-t border-line">
      ...
      <div className="mx-auto flex w-full max-w-[1200px] flex-wrap items-baseline justify-between gap-4 px-4 pt-14 sm:px-6">
        ...
        <Link
          href="/#contacto"
          className="rounded-(--radius-s) press bg-signal px-5 py-2.5 font-semibold text-void transition hover:brightness-110"
        >
          {t.footer.ctaAction}
        </Link>
      </div>
      ...
      <div className="mx-auto grid w-full max-w-[1200px] gap-10 px-4 py-14 sm:grid-cols-3 sm:px-6">
        ...
        <div className="flex flex-col items-start gap-1 font-mono text-xs text-dim">
          {/* solo enlaces reales; el estado pendiente va diseñado, no en corchetes */}
          {(
            [
            ...
            .map(([label, url]) => (
              <a key={label} href={url} target="_blank" rel="noopener noreferrer" className="link-draw tap-pad hover:text-ink">
                {label} →
              </a>
            ))}
          {isReal(site.email) ? (
            <a href={`mailto:${site.email}`} className="link-draw tap-pad hover:text-ink">
              {site.email}
            </a>
          ) : (
            <>
      ...
      {/* wordmark recortado por el borde (referencia WAIRK): tinta sutil sobre void.
          Con deriva GSAP: "emerge" un poco al llegar al final de la página. */}
